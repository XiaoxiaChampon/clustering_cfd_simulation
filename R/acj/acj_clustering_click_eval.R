...
ComputeClickClustering <- function(W_mat) 
{
  ...
  #   > clickstreams_test
  #   [1] "User1,h,c,c,p,c,h,c,p,p,c,p,p,o"
  #   [2] "User2,i,c,i,c,c,c,d"            
  #   [3] "User3,h,i,c,i,c,p,c,c,p,c,c,i,d"
  #   [4] "User4,c,c,p,c,d"                
  #   [5] "User5,h,c,c,p,p,c,p,p,p,i,p,o"  
  #   [6] "User6,i,h,c,c,p,p,c,p,c,d"      
  #   > str(clickstreams_test)
  #   chr [1:6] "User1,h,c,c,p,c,h,c,p,p,c,p,p,o" ...
  #   
  #   This i what you have: you need to change this list clickstream_data (you got from W) to 3861 characters and split each category value with , 
  #   str(clickstream_data)
  #   List of 3861
  #   $ 1   : chr "111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111"| __truncated__
  #   $ 2   : chr "111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111"| __truncated__
  #   $ 3   : chr "1121111212121111111112111111121111111111111111111111121111
  # 
  # (2) # run EM on the data
  # emclust <- click.EM(clickclust_data$X, K = 2)
  # 
  # # show(emclust)
  # W_label_clickclust <- emclust$id. 
  # 
  # Instead of using fixed K=2. use for loop K=2:5, then return emclust$BIC, pick the K that has the smallest BIC, then the final label will be the emclust$id for that specific K.
  #
  #
  # For clusterClickstreams
  # 
  # We use the same approach for click.EM. We use for loop for pre-defined the number of centers from 2:5, then return the totss, use K with the smallest totss as final K? Record that label as the final label. 
  # 
  # clusters <- clusterClickstreams(clickstream_data, order = 0, centers = 3)
  # clusters$totss.  compare this. from centers range from 2:5.
  # Then we use the final label of that number of centers.
  #
